...
class CTProjector(Operator):
    ...
    def SIRT(self, y: torch.Tensor, num_iters: int = 50) -> torch.Tensor:
        """Performs SIRT reconstruction using ASTRA on the stored geometry."""
        input_device = y.device
        input_dtype = y.dtype
        batch_size = y.shape[0]
        algo_name = "SIRT_CUDA" if self.use_gpu else "SIRT"

        results = []
        for i in range(batch_size):
            y_slice = y[i : i + 1]
            y_slice_np = y_slice.reshape(self.mx, self.my).detach().cpu().numpy()

            sino_id = astra.data2d.create("-sino", self.proj_geom, y_slice_np)
            ...
            try:
                ...
                astra.algorithm.run(alg_id, int(num_iters))
                x_recon_np = astra.data2d.get(recon_id)
            finally:
        ...
        return torch.cat(results, dim=0).to(input_device)
